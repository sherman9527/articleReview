...
3. **目录** — 锚点跳转
4. **一、文档结构分析** — 章节列表 + 结构问题
5. **二、敏感词与合规** — 按级别（L1/L2/L3）分组，含位置
6. **三、语言质量** — 问题卡片（原文划线 / 建议修改对比）
7. **四、引文检查** — 格式规范性
...
│   └── agents/
│       ├── base.py
│       ├── structure.py      # timeout 900s
│       ├── sensitive.py
│       ├── language.py       # timeout 900s，含成语误用规则
│       ├── citation.py       # timeout 1200s，GB/T 7714-2015
│       └── policy.py         # timeout 900s，含政治引用精确性
└── output/                   # 审核输出（不提交）
```
